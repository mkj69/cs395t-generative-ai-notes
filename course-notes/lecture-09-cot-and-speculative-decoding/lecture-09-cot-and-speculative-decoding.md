...
- `[SOURCE]` If the hinted generation now ends at `y_i`, its rationale `r_i^rat` is retained.
- `[EXPLANATION]` During fine-tuning, the answer hint is removed from the input. The training pair is therefore question `x_i` -> rationale `r_i^rat` followed by answer `y_i`.
...
- `[DERIVATION · Step 1]` For one training question, expand the expectation over every possible generated rationale `r` and answer `y`:
  ...
  ```text
  J_i(theta) = sum_{r,y} p_theta(r,y | x_i) 1{y = y_i}.
  ```
...
- `[DERIVATION · Step 2]` Differentiate term by term:
  ...
  ```text
  grad J_i(theta)
    = sum_{r,y} 1{y = y_i} grad p_theta(r,y | x_i).
  ```
- `[DERIVATION · Step 3]` Use the log-derivative identity `grad p = p grad log p`:

  ```text
  grad J_i(theta)
    = sum_{r,y} p_theta(r,y | x_i) 1{y = y_i}
        grad log p_theta(r,y | x_i).
  ```
- `[DERIVATION · Step 4]` Recognize the weighted sum as an expectation under the current model:

  ```text
  grad J(theta)
    = sum_i E_{(r_hat,y_hat) ~ p_theta(.|x_i)}[
        1{y_hat = y_i} grad log p_theta(r_hat,y_hat | x_i)
      ].
  ```
...
- `[SOURCE]` Put the input representations into `X = [x_1, ..., x_N]`, with every `x_j` in `R^d`. For attention head `h`, define
...
- `[SOURCE]` At output position `i`, head `h` forms
  ...
  ```text
  y_i^(h) = sum_j A_ji^(h) v_j^(h),

  A_ji^(h) = exp(<q_i^(h), k_j^(h)> / sqrt(d))
             / sum_u exp(<q_i^(h), k_u^(h)> / sqrt(d)).
  ```
...
- `[EXAMPLE]` If `g(person)` returns that person's parent and `f(person)` returns that person's occupation, a query about the occupation of Alice's parent first needs the identity `g(Alice)` and then the value `f(g(Alice))`.
- #### Single-layer composition lower bound (Theorem 1.1)
- `[THEOREM]` With `H` heads, hidden dimension `d`, and `b` bits per scalar, define `B = H(d + 1)b`. When `B < n log_2 n`, set `R = n log_2 n - B > 0`. For independent uniformly random tables `f` and `g`, every fixed query `a` has error probability at least `R / (3 n log_2 n)`. Averaging gives the same bound for an independent uniform query.
...
- `[DEFINITION]` For a discrete random variable `U`, `H(U) = -sum_u P(U=u) log P(U=u)`.
- `[DEFINITION]` Conditional mutual information is `I(U;V | W) = H(U | W) - H(U | V,W)`.
- `[DEFINITION]` Binary entropy is `h_2(delta) = -delta log delta - (1-delta) log(1-delta)`.
- #### Fano's inequality (Lemma 1.2)
- `[THEOREM]` Let `Y` take values in `[n]`, let `Z` be an observation, and let `Y_hat = psi(Z)`. If `delta = P(Y_hat != Y)`, then

  ```text
  H(Y | Z) <= h_2(delta) + delta log(n - 1).
  ```
- `[DERIVATION · Step 1]` Define the error flag `E = 1{Y_hat != Y}`. Once `(Y,Z)` is known, `Y_hat = psi(Z)` is known, so `E` is determined. Therefore `H(E | Y,Z) = 0`.
- `[DERIVATION · Step 2]` Add `E` without changing the conditional uncertainty:

  ```text
  H(Y | Z) = H(E,Y | Z).
  ```
- `[EXPLANATION]` The chain rule normally gives `H(E,Y|Z) = H(Y|Z) + H(E|Y,Z)`. The final term is zero by Step 1.
- `[DERIVATION · Step 3]` Apply the chain rule in the other order:

  ```text
  H(E,Y | Z) = H(E | Z) + H(Y | E,Z).
  ```
- `[DERIVATION · Step 4]` Conditioning cannot increase entropy, so `H(E | Z) <= H(E) = h_2(delta)`.
- `[DERIVATION · Step 5]` When `E=0`, the observation determines `Y` because `Y = Y_hat`. When `E=1`, `Y` can be any label except `Y_hat`, leaving at most `n-1` possibilities. Hence

  ```text
  H(Y | E,Z) <= delta log(n - 1).
  ```
- `[CONCLUSION]` Adding the two bounds proves Fano's inequality.
- #### Indexing communication lemma (Lemma 1.3)
- `[SETUP]` Faye knows a uniformly random table `F = (F_1,...,F_n)` in `[n]^n`. She sends a `B`-bit message `M` before Xavier receives an independent uniform index `J`. Xavier must estimate `F_J` from `(M,J)`.
- `[THEOREM]` If `B < n log n` and `R = n log n - B`, every such protocol has error at least `R / (3 n log n)`.
- `[DERIVATION · Step 1]` Because `J` is uniform and independent,

  ```text
  I(M; F_J | J) = (1/n) sum_{j=1}^n I(M; F_j).
  ```
- `[DERIVATION · Step 2]` The coordinates of `F` are independent, so `H(F_j) = H(F_j | F_<j)`. Also, adding `F_<j` to the conditioning can only reduce the remaining entropy of `F_j`. Thus

  ```text
  I(M;F_j) <= I(M;F_j | F_<j).
  ```
- `[DERIVATION · Step 3]` Sum the preceding terms and use the mutual-information chain rule:

  ```text
  I(M;F_J | J)
    <= (1/n) sum_j I(M;F_j | F_<j)
     = (1/n) I(M;F)
    <= H(M)/n
    <= B/n.
  ```
- `[EXPLANATION]` A `B`-bit message has at most `2^B` values, so its entropy cannot exceed `B`.
- `[DERIVATION · Step 4]` Before seeing `M`, the requested value is uniform on `[n]`, hence `H(F_J | J) = log n`. After seeing the message,
...
- `[DERIVATION · Step 5]` Apply Fano to Xavier's estimate:
  ...
  ```text
  R/n <= h_2(delta) + delta log(n - 1).
  ```
- #### Turning Fano's bound into the stated linear bound
- `[CASE 1]` Suppose `delta >= 3/(4n)`. Use `h_2(u) <= u log(e/u)`:

  ```text
  R/n <= delta log(e(n-1)/delta) <= 3 delta log n.
  ```
- `[EXPLANATION]` The last comparison follows from `e(n-1)/delta <= (4e/3)n(n-1) <= n^3` for `n >= 2`. Rearranging yields `delta >= R/(3n log n)`.
- `[CASE 2]` Suppose `delta < 3/(4n)`. Each message `m` determines a full predicted table `F_hat(m)` by collecting Xavier's answers for all possible indices. Since there are at most `2^B` messages, at most `2^B` of the `n^n` possible tables can be recovered perfectly.
- `[DERIVATION]` A wrong table differs in at least one coordinate. Averaging over the uniform requested coordinate gives
  ...
  n delta >= P(F != F_hat(M))
          >= 1 - 2^B / n^n
           = 1 - 2^(-R).
  ```
- `[DERIVATION]` The assumption `n delta < 3/4` forces `R < 2`. On `0 <= R <= 2`, concavity gives `1 - 2^(-R) >= 3R/8`. Therefore

  ```text
  delta >= 3R/(8n) >= R/(3n log n).
  ```
...
- `[DERIVATION · Step 3]` For head `h`, define unnormalized attention weight `w_j^(h) = exp(<q_t^(h),k_j^(h)>/sqrt(d))`. The holder of `f` computes

  ```text
  S_f^(h) = sum_{j in I_f} w_j^(h) v_j^(h)   in R^d,
  Z_f^(h) = sum_{j in I_f} w_j^(h)           in R.
  ```
...
- `[SOURCE]` For functions `f_1,...,f_K`, set `z_0 = a` and recursively define `z_j = f_j(z_{j-1})`. The desired answer is `z_K`.
- `[SOURCE]` The lecture states, without proving, that sufficiently general iterated composition has a worst-case lower bound of `Omega(sqrt(n / (H d b)))` CoT steps in this finite-precision model. The underlying argument uses pointer-chasing communication complexity.
...
- `[DEFINITION]` If the draft tokens are `v_tilde_1,...,v_tilde_gamma`, define

  ```text
  q_i(.) = q(. | s v_tilde_1 ... v_tilde_{i-1}),
  p_i(.) = p(. | s v_tilde_1 ... v_tilde_{i-1}).
  ```
...
- `[ALGORITHM · Draft]` For `i = 1,...,gamma`, sample `v_tilde_i ~ q_i`. These draft steps remain sequential because proposal `i` conditions on earlier proposals.
- `[ALGORITHM · Verify]` Evaluate `p_1,...,p_{gamma+1}` together with the target model.
- `[ALGORITHM · Test]` At position `i`, draw a fresh `U_i ~ Uniform[0,1]`. Accept `v_tilde_i` when

  ```text
  U_i <= min(1, p_i(v_tilde_i) / q_i(v_tilde_i)).
  ```
- `[ALGORITHM · Correct]` At the first rejection, form
  ...
  r_i(v) = [p_i(v) - q_i(v)]_+
           / sum_u [p_i(u) - q_i(u)]_+.
  ```
- `[ALGORITHM · Return after rejection]` Sample `w ~ r_i`, emit the earlier accepted drafts followed by `w`, and discard every later draft token.
...
- `[SETUP]` Ignore prefixes temporarily and consider two distributions `p` and `q` on the same finite vocabulary. Propose `V ~ q`, accept with probability `min(1,p(V)/q(V))`, and on rejection draw from the residual distribution.
- `[DERIVATION · Step 1]` The probability that value `v` is both proposed and accepted is

  ```text
  q(v) min(1, p(v)/q(v)) = min(p(v), q(v)).
  ```
- `[EXPLANATION]` If `p(v) >= q(v)`, every proposal of `v` is accepted and contributes `q(v)`. If `p(v) < q(v)`, only the fraction `p(v)/q(v)` is accepted and contributes `p(v)`. If `q(v)=0`, `v` is never proposed, so the ratio is never evaluated on that event.
...
- `[DERIVATION · Step 3]` Use `min(a,b) = (a+b-|a-b|)/2` and the fact that both distributions sum to one:
  ...
  ```text
  beta = 1 - (1/2) sum_v |p(v)-q(v)|
       = 1 - TV(p,q).
  ```
- `[EXPLANATION]` The rejection probability is therefore `1-beta = TV(p,q)`. A better draft model overlaps more strongly with the target and is rejected less often.
...
- `[THEOREM]` Repeated speculative decoding produces exactly the target autoregressive law. For target length `T`, the probability of tokens `(v_1,...,v_T)` after prompt `s_0` is

  ```text
  product_{t=1}^T p(v_t | s_0 v_1 ... v_{t-1}).
  ```
...
- `[DEFINITION]` At prefix `s`, define the overlap
  ...
  ```text
  beta(s) = sum_v min(p(v | s), q(v | s)).
  ```
- `[EXPLANATION]` By the one-token lemma, `beta(s)` is exactly the probability that a draft proposal at prefix `s` is accepted.
- `[DEFINITION]` Let `A_i` be the event that proposal `i` passes. Let `R_i = A_1 cap ... cap A_{i-1}` be the event that position `i` is reached, with `R_1` always true. Let `S_i` be the random draft prefix present at that position.
- `[ASSUMPTION]` For the runtime calculation, suppose every reached position has the same conditional mean overlap:

  ```text
  E[beta(S_i) | R_i] = alpha.
  ```
- `[DERIVATION]` Conditional on `S_i`, the probability of passing is `beta(S_i)`. Taking the conditional expectation over reached prefixes gives `P(A_i | R_i) = alpha`.
...
- `[DERIVATION · Step 1]` Reaching at least `j` acceptances means that the first `j` acceptance events all occurred:
  ...
  ```text
  P(L >= j) = P(A_1 cap ... cap A_j).
  ```
- `[DERIVATION · Step 2]` Repeated conditioning uses `P(A_i | R_i)=alpha` at every reached position:

  ```text
  P(L >= j) = alpha^j,     0 <= j <= gamma.
  ```
...
- `[DERIVATION · Step 3]` For an integer-valued variable `N` in `{1,...,gamma+1}`, use the tail-sum identity

  ```text
  N = sum_{k=1}^{gamma+1} 1{N >= k}.
  ```
- `[DERIVATION · Step 4]` Take expectations and substitute the tail probabilities:

  ```text
  E[N] = sum_{j=0}^{gamma} P(L >= j)
       = sum_{j=0}^{gamma} alpha^j
       = (1 - alpha^(gamma+1)) / (1-alpha),   alpha < 1.
  ```
- `[EDGE CASE]` If `alpha=1`, every proposal is accepted and `E[N]=gamma+1`.
- #### Cost model
...
- `[DERIVATION · Step 2]` Speculative decoding spends `(1+gamma c)T_p` per iteration and emits `E[N]` tokens on average, so its long-run time per token is
...
- `[THEOREM]` Let `U` be uniform on a nonempty set `S subseteq [n]^n`. For any fixed vector `z in [n]^n`,
  ...
  ```text
  log |S| <= 3 log n E[d_H(U,z)].
  ```
- `[DEFINITION]` `d_H(U,z)` counts the coordinates where `U` and `z` differ. Write `s=|S|` and `D=E[d_H(U,z)]`.
- `[CASE s=1]` The left side is zero, so the inequality follows from `D>=0`.
- `[CASE s=2 or 3]` At most one element of `S` can equal `z`; therefore `D >= (s-1)/s`. Directly checking `s=2,3` gives `log s <= 3(s-1)/s <= 3D log n`.
- `[CASE s>=4 · Step 1]` The same observation gives `D >= (s-1)/s >= 3/4`.
- `[CASE s>=4 · Step 2]` Let `e_j = P(U_j != z_j)`. To describe coordinate `U_j`, first record whether it equals `z_j`; if not, record one of at most `n-1` alternatives. Therefore

  ```text
  H(U_j) <= h_2(e_j) + e_j log(n-1).
  ```
- `[CASE s>=4 · Step 3]` Because `U` is uniform on `S`, `H(U)=log s`. Entropy subadditivity and concavity of `h_2` yield
...
- `[CASE s>=4 · Step 4]` Apply `h_2(u) <= u log(e/u)`:

  ```text
  log s <= D log(e n(n-1)/D).
  ```
- `[CASE s>=4 · Step 5]` Since `D>=3/4` and `n>=2`, the logarithm's argument is at most `n^3`, giving `log s <= 3D log n`.
...
- `[CONCLUSION]` Average the per-message upper bound and compare it with the lower bound:
  ...
  ```text
  R <= H(F|M) <= 3 n log n delta.
  ```
- `[CONCLUSION]` Therefore `delta >= R/(3n log n)`.
